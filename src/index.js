...
import { path, caniuselite } from "../deps.js";
...
function filterByYear(since, context) {
  since = since / 1000;
  return Object.keys(caniuselite.agents).reduce(function (selected, name) {
  ...
  return queries.reduce(async function (result, query, index) {
    ...
    for (var i = 0; i < QUERIES.length; i++) {
      ...
      if (match) {
        ...
        var array = await Promise.resolve(
          type.select.apply(browserslist, args),
        );
...
export async function browserslist(queries, opts) {
  ...
  var result = uniq(await resolve(queries, context)).sort(
...
(function () {
  for (var name in caniuselite.agents) {
    var browser = caniuselite.agents[name];
    browserslist.data[name] = {
      name: name,
      versions: normalize(caniuselite.agents[name].versions),
      released: normalize(caniuselite.agents[name].versions.slice(0, -3)),
      releaseDate: caniuselite.agents[name].release_date,
    };
